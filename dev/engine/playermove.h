...
#else
#if defined (PLAYER_CAN_FIRE) || defined (PLAYER_CAN_PUNCH) || defined (PLAYER_HAZ_SWORD) || defined (PLAYER_CAN_DRILL) || defined (CARRIABLE_BOXES_THROWABLE)
...
#include "engine/playermods/fire_button.h"	
	...
	gpxx = (gpx + 8) >> 4;
	gpyy = (gpy + 8) >> 4;
	if (qtile (gpxx, gpyy) == HELP_TILE_GET) {
		gpaux = gpxx + (gpyy << 4) - gpyy;
		map_buff [gpaux] = 0;
		map_attr [gpaux] = 0;
		draw_coloured_tile_gamearea (gpxx, gpyy, 0);
	...
	gpxx = (gpx + 8) >> 4;
	gpyy = (gpy + 8) >> 4;
	if (qtile (gpxx, gpyy) == LIVE_TILE_GET && p_life < 99) {
		gpaux = gpxx + (gpyy << 4) - gpyy;
		map_buff [gpaux] = 0;
		map_attr [gpaux] = 0;
		draw_coloured_tile_gamearea (gpxx, gpyy, 0);
		p_life ++;
#ifdef PERSISTENT_LIVE_TILE_GET
		map_pointer = map + n_pant * 75 + (gpaux >> 1);
		*map_pointer = (gpaux & 1) ? ((*map_pointer) & 0xf0) : ((*map_pointer) & 0x0f);
#endif		
#ifdef MODE_128K
